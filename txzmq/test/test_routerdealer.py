...
class ZmqRouterDealerTwoFactoryConnectionTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dealer_endpoint = ZmqEndpoint(ZmqEndpointType.connect, "tcp://127.0.0.1:7859")
        self.dealer = ZmqTestDealerConnection(self.factory1, dealer_endpoint, identity='dealer')
        self.dealer.d = defer.Deferred()

        self.factory2 = ZmqFactory()
        router_endpoint = ZmqEndpoint(ZmqEndpointType.bind, "tcp://127.0.0.1:7859")
        self.router = ZmqTestRouterConnection(self.factory2, router_endpoint, identity='router')
